...
fs.copyFileSync(
  queueFile,
  `${queueFile}.before-package-13g-20-fix-1.bak`,
);

if (fs.existsSync(schedulerTarget)) {
  fs.copyFileSync(
    schedulerTarget,
    `${schedulerTarget}.before-package-13g-20-fix-1.bak`,
  );
}
...
const stateBlock = `const SCHEDULER_INTERVAL_MS = 30000;
...
if (!source.includes(stateBlock)) {
  throw new Error(
    'Scheduler durum bloğu bulunamadı.',
  );
}
...
// Scheduler claim set artık scheduler servisinde temizleniyor.
source = source.replace(
  `  runtimeStates.clear();
...
console.log('13G-20-FIX-1 başarıyla uygulandı.');
