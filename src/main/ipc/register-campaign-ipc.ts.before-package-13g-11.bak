...
function reportCampaignStartupHealthSummary(): void {
  const database = getDatabase();
  ...
  try {
    ...
    const missingSettingsCount = Number(
      database
        .prepare(
          `SELECT COUNT(*)
    ...
    console.info(
      `[Campaign Engine] Başlangıç özeti: kampanya=${Number(campaignRow.total ?? 0)}, running=${Number(campaignRow.running ?? 0)}, scheduled=${Number(campaignRow.scheduled ?? 0)}, paused=${Number(campaignRow.paused ?? 0)}.`,
    );

    console.info(
      `[Campaign Engine] Alıcı özeti: toplam=${Number(recipientRow.total ?? 0)}, pending=${Number(recipientRow.pending ?? 0)}, sending=${Number(recipientRow.sending ?? 0)}, başarılı=${Number(recipientRow.successful ?? 0)}, failed=${Number(recipientRow.failed ?? 0)}.`,
    );

    if (
      Number(recipientRow.invalid_phone ?? 0) > 0 ||
      orphanRecipientCount > 0 ||
      missingSettingsCount > 0
    ) {
      console.warn(
        `[Campaign Engine] Veri uyarısı: geçersiz telefon=${Number(recipientRow.invalid_phone ?? 0)}, sahipsiz alıcı=${orphanRecipientCount}, ayarsız kampanya=${missingSettingsCount}.`,
      );
    }
  } catch (reason: unknown) {
    const message =
    ...
    console.error(
      `[Campaign Engine] Başlangıç sağlık özeti başarısız: ${message}`,
    );
  }
}
