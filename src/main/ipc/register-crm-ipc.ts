...
interface ContactRow {
  ...
  city: string | null;
  phone_number: string;
  permission_status: CrmPermissionStatus;
  notes: string | null;
  created_at: string;
  updated_at: string;
}
...
interface NormalizedImportRow {
  ...
  city: string | null;
  phoneNumber: string;
  status: CrmImportPreviewRow['status'];
  issue: string | null;
}
...
function normalizePhone(value: string): string {
  ...
  if (phone.length < 10 || phone.length > 15) {
    throw new Error(
      'Telefon numarasını ülke koduyla birlikte 10-15 hane olarak girin.',
    );
  }
...
function assertPermission(
  value: CrmPermissionStatus | undefined,
): CrmPermissionStatus {
...
function listContacts(
  ...
  const permissionStatus =
    request.permissionStatus === 'allowed' ||
  ...
  const rows = database
    .prepare(`
      ...
      LIMIT ? OFFSET ?
    `)
    .all(...parameters, limit, offset) as ContactRow[];
...
function saveContact(input: CrmContactSaveInput): CrmContact {
  ensureCrmSchema();
  ...
  const existingPhone = database
    .prepare(`
      ...
      LIMIT 1
    `)
    .get(phoneNumber, id) as { id: string } | undefined;

  if (existingPhone) {
    throw new Error(
      'Bu telefon numarası başka bir CRM kişisinde kayıtlı.',
    );
  }
  ...
  `).run(
    ...
    cleanOptional(input.sector),
    cleanOptional(input.city),
    phoneNumber,
    assertPermission(input.permissionStatus),
...
function deleteContact(
  request: CrmContactDeleteRequest,
): { deleted: boolean } {
  ensureCrmSchema();
...
function cleanupExpiredImportSessions(): void {
  const now = Date.now();
...
async function previewImport(): Promise<CrmImportPreview | null> {
  ensureCrmSchema();
  ...
  const companyNameKey = findHeaderKey(
    headers,
    HEADER_ALIASES.companyName,
  );
  const sectorKey = findHeaderKey(headers, HEADER_ALIASES.sector);
  const cityKey = findHeaderKey(headers, HEADER_ALIASES.city);
  const phoneNumberKey = findHeaderKey(
    headers,
    HEADER_ALIASES.phoneNumber,
  );

  if (!fullNameKey || !phoneNumberKey) {
    throw new Error(
      'Dosyada Ad Soyad ve Telefon sütunları bulunamadı. Sütun başlıklarını kontrol edin.',
    );
  }
  ...
  const existingPhones = new Set(
    (
      database
        ...
        .all() as Array<{ phone_number: string }>
  ...
  sourceRows.forEach((sourceRow, index) => {
    ...
    const companyName = cleanOptional(
      readCell(sourceRow, companyNameKey),
    );
    ...
    let status: NormalizedImportRow['status'] = 'new';
...
function applyImport(
  ...
  if (!session) {
    throw new Error(
      'İçe aktarma önizlemesinin süresi doldu. Dosyayı yeniden seçin.',
    );
  }
  ...
  const transaction = database.transaction(() => {
    for (const row of session.rows) {
      ...
      const existing = findExisting.get(row.phoneNumber) as
        | { id: string }
        | undefined;
...
async function exportContacts(
  ...
  const exportRows = rows.map((row) => ({
    ...
    'İzin Durumu':
      row.permission_status === 'allowed' ? 'İzinli' : 'Engelli',
    Notlar: row.notes ?? '',
...
export function registerCrmIpcHandlers(): void {
  ensureCrmSchema();
  ...
  ipcMain.handle(
    IPC_CHANNELS.list,
    (_event, request: CrmContactListRequest) =>
      listContacts(request),
  );

  ipcMain.handle(
    IPC_CHANNELS.save,
    (_event, input: CrmContactSaveInput) =>
      saveContact(input),
  );

  ipcMain.handle(
    IPC_CHANNELS.delete,
    (_event, request: CrmContactDeleteRequest) =>
  ...
  ipcMain.handle(
    IPC_CHANNELS.applyImport,
    (_event, request: CrmImportApplyRequest) =>
      applyImport(request),
  );

  ipcMain.handle(
    IPC_CHANNELS.bulkPermission,
    (_event, request: CrmBulkPermissionRequest) =>
      bulkUpdatePermission(request),
  );

  ipcMain.handle(
    IPC_CHANNELS.bulkDelete,
    (_event, request: CrmBulkDeleteRequest) =>
      bulkDeleteContacts(request),
  );

  ipcMain.handle(
    IPC_CHANNELS.export,
    (_event, request: CrmExportRequest) =>
