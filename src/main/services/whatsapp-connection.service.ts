...
function randomTypingDuration(): number {
...
export async function sendCampaignMessage(
  ...
  try {
    ...
    throw new Error(
      `Kampanya mesajı gönderilemedi: ${message}`,
    );
  }
}
...
export async function deleteWhatsAppChat(
  context: InboxChatDeleteContext,
): Promise<void> {
  ...
  if (!context.lastMessage?.id?.trim()) {
    throw new Error(
      'WhatsApp sohbetini silmek için son mesaj kimliği bulunamadı.',
    );
  }
  ...
  try {
    ...
    console.log(
      `[Inbox] ${accountId} WhatsApp sohbeti silindi | ` +
    ...
    console.error(
      `[Inbox] ${accountId} WhatsApp sohbeti silinemedi | ` +
        `jid=${jid} hata=${message}`,
    );

    throw new Error(
      `WhatsApp sohbeti silinemedi: ${message}`,
    );
  }
}
