...
import type { CampaignRuntimeState } from './campaign-runtime-state.service.js';
...
export async function runCampaignQueue(input: {
  ...
  if (!canStartCampaignRuntime(state, expectedRunId)) return;
  state.running = true;
  ...
  try {
    while (!state.cancelled) {
      ...
      if (state.paused) {
        ...
        if (persistedCampaign.status === 'running') state.paused = false;
      ...
      if (campaign.status === 'paused') { state.paused = true; continue; }
      ...
      if (!campaign.accountId) throw new Error('Kampanya gönderen WhatsApp hesabı seçilmemiş.');
    ...
    try { reconcileCampaignState(campaignId); }
    catch (reason: unknown) {
    ...
    if (state.cancelled) runtimeStates.delete(campaignId);
    else {
      try {
