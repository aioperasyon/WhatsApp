...
export function getWhatsAppAccount(
  ...
  const row = getDatabase()
    .prepare(`
      ...
      WHERE id = ?
    `)
    .get(accountId) as WhatsAppAccountRow | undefined;

  return row ? mapAccount(row) : null;
}

export function listWhatsAppAccounts(): WhatsAppAccount[] {
...
export function updateWhatsAppAccountConnection(
  accountId: string,
  values: {
    status: WhatsAppAccountStatus;
  ...
  const lastConnectedAt =
    values.lastConnectedAt === undefined
...
export function deleteWhatsAppAccount(
  accountId: string,
): DeleteWhatsAppAccountResult {
