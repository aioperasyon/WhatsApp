...
const repositorySource =
  'files/src/main/repositories/campaign-state.repository.ts';
const repositoryTarget =
  'src/main/repositories/campaign-state.repository.ts';
...
fs.copyFileSync(
  queueFile,
  `${queueFile}.before-package-13g-19.bak`,
);

if (fs.existsSync(repositoryTarget)) {
  fs.copyFileSync(
    repositoryTarget,
    `${repositoryTarget}.before-package-13g-19.bak`,
  );
}
...
source = removeBetween(
  ...
  'async function waitForCampaignRuntime(',
  'claimCampaignStart',
);
...
if (!source.includes(oldRuntimeLookup)) {
  throw new Error(
    'Runtime status sorgusu bulunamadı.',
  );
}
...
const newCompleteBlock = `        completeCampaign(campaignId);`;

if (!source.includes(oldCompleteBlock)) {
  throw new Error(
    'Kampanya tamamlama sorgusu bulunamadı.',
  );
}
...
console.log('13G-19 başarıyla uygulandı.');
