...
import { getInboxService, shutdownInboxServices, } from './services/inbox-container.service.js';
import { shutdownCampaignEngine, } from './services/campaign-queue.service.js';
import { closeAllWhatsAppConnections } from './services/whatsapp-connection.service.js';
const currentFile = fileURLToPath(import.meta.url);
const currentDirectory = path.dirname(currentFile);
const applicationUserDataDirectory = app.getPath('userData');
const electronCacheDirectory = path.join(applicationUserDataDirectory, 'electron-cache');
...
void app.whenReady().then(() => {
    ...
    console.log(`[bootstrap] databaseReady=${bootstrapResult.databaseReady} schemaVersion=${bootstrapResult.schemaVersion}`);
...
app.on('before-quit', (event) => {
    ...
    void (async () => {
        try {
            ...
            console.error(`[shutdown] Kontrollü kapanış tamamlanamadı: ${message}`);
        }
        finally {
            closeDatabase();
            app.quit();
