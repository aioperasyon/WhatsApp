...
export class InboxRepository {
    ...
    `).run(targetChat.id, newJid, now, accountId, oldChat.id);
      ...
      SET
        ...
        updated_at = ?
      WHERE id = ? AND account_id = ?
    `).run(oldChat.display_name, phoneNumber, oldChat.unread_count, oldChat.last_message_at, oldChat.last_message_preview, oldChat.last_message_at, oldChat.last_message_at, now, targetChat.id, accountId);
        this.database
    ...
    upsertChat(input) {
      ...
      `).run(id, input.accountId, input.jid, input.displayName ?? null, input.phoneNumber ?? null, input.lastMessagePreview ?? null, input.lastMessageAt ?? null, input.incrementUnread ? 1 : 0, now, now);
      ...
      SET
        ...
        updated_at = ?
      WHERE id = ? AND account_id = ?
    `).run(input.displayName ?? null, input.phoneNumber ?? null, input.lastMessagePreview ?? null, input.lastMessageAt ?? null, input.incrementUnread ? 1 : 0, now, existing.id, input.accountId);
    ...
    saveMessage(input) {
        if (input.whatsappMessageId) {
            const existing = this.database
                ...
                .get(input.accountId, input.whatsappMessageId);
            if (existing)
    ...
    `).run(id, input.accountId, input.chatId, input.remoteJid, input.whatsappMessageId ?? null, input.direction, input.senderJid ?? null, input.text ?? null, input.messageType, input.status, input.timestamp, now, now);
    ...
    listChats(request) {
      ...
      ORDER BY
        ...
        updated_at DESC
      LIMIT ? OFFSET ?
    `).all(request.accountId, unreadOnly, search, like, like, like, like, limit, offset);
    ...
    `).get(request.accountId, unreadOnly, search, like, like, like, like);
    ...
    listMessages(request) {
        ...
        LIMIT ?
      `).all(request.accountId, request.chatId, request.before, limit);
      ...
      ORDER BY timestamp DESC
      LIMIT ?
    `).all(request.accountId, request.chatId, limit);
        return rows.reverse().map(mapMessage);
    }
    ...
    getMessageById(accountId, messageId) {
        ...
        if (!row)
            throw new Error('Mesaj kaydı bulunamadı.');
